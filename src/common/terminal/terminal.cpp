...
static size_t getIdx( size_t i, size_t j )
{
	return instance->VGA_WIDTH * j + (i + instance->indent);
}

void terminal_putentryat(char c, uint8_t color, size_t x, size_t y) 
{
	const size_t index = getIdx(x, y);
...
void ckTerminal_scrdown( ckTerminal_t *term )
{
	for (size_t i=0; i<term->VGA_WIDTH; i++)
	{
		...
		{
			size_t idx0 = getIdx(i, j+0);
			size_t idx1 = getIdx(i, j+1);
...
ckTerminal_putc( char c )
{
	...
	size_t idx = getIdx(row, col); //  row*instance->VGA_WIDTH + col;
