...
// extern "C" void _nasm_gdt_load( uint32_t a, uint16_t b );
...
// void ckGDT32_create( ck_gdt_table *GDT, ck_TSS *tss )
{
    ...
    // uint32_t ptr = reinterpret_cast<uint32_t>(GDT); // get the pointer to your GDT table as a 32bit int

    GDTR gdtr = { 3*sizeof(uint64_t)-1, reinterpret_cast<uint64_t>(GDT) };
