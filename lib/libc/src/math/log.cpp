...
{
    return (((pow(x, 0.00001) - 1.0) / 0.00001) * M_LOG10E);
}
...
double logx( double x, double base )
{
    ...
    if(base == 1.0 || base < 0.0)
        return 0.0; // Return what you want here, could be Not-A-Number or NAN.
...
std::pair<double, int> my_frexp( double x )
{
    ...
    return std::pair<double, int>(frac, exponent+1);
}
...
static std::pair<float, int> reduce_fp32(float x);
static double recip_of_masked(float x);
static double log_recp_of_masked(float x);
static double approximate_log_pol_1_to_1001(double x);
...
double logn( double value )
{
    ...
    if (x == 0)
        return std::bit_cast<float, unsigned>(0xff800000); // -Inf

    else if (x < 0)
        return std::bit_cast<float, unsigned>(0xffc00000); // -Nan.
    ...
    double z  = m * ri - 1;
    double L2 = std::bit_cast<double, uint64_t>(0x3fe62e42fefa39ef);
    ...
    return (double)((E*L2 + ln_1z) - ln_ri);
}
...
static std::pair<float, int> reduce_fp32(float x)
{
    uint32_t bits = std::bit_cast<uint32_t, float>(x);
    if (bits == 0) {
        return { 0., 0 };
    }
    // See:
    ...
    float frac = std::bit_cast<float, uint32_t>(res);
...
static double recip_of_masked(float x)
{
    ...
    return std::bit_cast<double, uint64_t>(bval);
}
