...
{
    uint32_t eax, edx;
    CPU::getMSR(IA32_APIC_BASE_MSR, &eax, &edx);
...
void cpuSetAPICBase(uintptr_t apic) 
{
    ...
    CPU::setMSR(IA32_APIC_BASE_MSR, eax, edx);
}
...
static uint32_t checkAPIC(void)
{
...
{
    syslog log("enableAPIC");

    log("checkAPIC: %u", checkAPIC());
    log("Enabling APIC");
    ...
    PIC::disable(); // disableAllIRQs();

    /* Hardware enable the Local APIC if it wasn't enabled */
    cpuSetAPICBase(cpuGetAPICBase());

    log("printNumber: %lu", cpuGetAPICBase());
...
// APIC TIMER
static void APIC_timer_callback( intframe_t* )
{
    // do timer stuff
...
void enableAPICTimer( uint32_t ms ) 
{
    PIT::init();
    PIT::set_ms(ms);
    ...
    // Sleep for 10 ms
    syslog log("enableAPICTimer");
    log("Calibrating APIC Timer");
    PIT::sleep(10);
    log("Finished Calibration");
    ...
    CPU::installIRQ(0, APIC_timer_callback);
}
