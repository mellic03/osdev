...
// static uint64_t global_rfs_buf[sizeof(RamFS)/8];
// static RamFS   *global_rfs;

// // void rfs::init( void *base )
// // {
// //     global_rfs = new RamFS(base);
// // }

// void rfs::init()
// {
//     size_t nbytes = 8*(1024*1024); // 8MB
//     size_t npages = (nbytes+PMM::PAGE_SIZE) / PMM::PAGE_SIZE;
//     uintptr_t phys = PMM::allocPages(npages);
//     uintptr_t virt = phys + PMM::hhdm;
//     VMM::mapRange(phys, virt, npages);

//     void *base = (void*)virt;
//     global_rfs = new (global_rfs_buf) RamFS(base, nbytes);

//     // void *base = kmalloc(nbytes);
//     // global_rfs  = (RamFS*)kmalloc(sizeof(RamFS));
//     // *global_rfs = RamFS(base, nbytes); // new (global_rfs) RamFS(base, nbytes);
// }
...
// void *rfs::open( const char *path )
// {
//     return (void*)(global_rfs->open(path));
// }

// size_t rfs::fwrite( void *file, const void *src, size_t nbytes )
// {
//     // syslog log("rfs::fwrite");
//     // log("file:   0x%lx", file);
//     // log("src:    0x%lx", src);
//     // log("nbytes: %lu",   nbytes);
//     return global_rfs->fwrite((rfsEntry*)file, src, nbytes);
// }

// size_t rfs::fread( void *file, void *dst, size_t nbytes )
// {
//     return global_rfs->fread((rfsEntry*)file, dst, nbytes);
// }
...
// size_t RamFS::fwrite( rfsEntry *file, const void *srcbuf, size_t nbytes )
// {
//     // syslog log("RamFS::fwrite");
//     // log("file:   0x%lx", file);
//     // log("impl:   0x%lx", file->impl);
//     // log("src:    0x%lx", srcbuf);
//     // log("nbytes: %lu",   nbytes);
//     // log("wcstm:  0x%lx", file->wcstm);
//     // log("flags:  %lu",   file->flags);

//     if (file->wcstm && (file->flags & rfsFlag_WriteCstm))
//     {
//         return file->wcstm(file, srcbuf, nbytes);
//     }
...
// size_t RamFS::fread( rfsEntry *file, void *dstbuf, size_t nbytes )
// {
//     if (file->rcstm && (file->flags & rfsFlag_ReadCstm))
//     {
//         return file->rcstm(file, dstbuf, nbytes);
//     }
