...
:	ID(id)
{
	...
	std::memset(m_buf, '\0', width*height);
}
...
void
idk::Terminal::putchar_at( int row, int col, char c )
{
	int prev = CSR;
	setcursor(row, col);
	putchar(c);
...
void 
idk::Terminal::putstr( const char *str )
{
	while (*str)
	{
		putchar(*(str++));
...
void 
idk::Terminal::putstr_at( int row, int col, const char *str )
{
	int prev = CSR;
	setcursor(row, col);
	putstr(str);
